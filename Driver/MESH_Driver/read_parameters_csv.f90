...
subroutine read_parameters_csv(shd, iun, fname, ierr)
    ...
    do while (z == 0)
        ...
        select case (uppercase(args(1)))
            ...
            case (VN_SVS_LWATER_PONDING_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    p = 1
                    call assign_line_args(svs_mesh%vs%lwater_ponding_svs, args(2), istat)
                end if
            case (VN_SVS_READ_OC)
                if (.not. svs_mesh%PROCESS_ACTIVE .or. svs_mesh%vs%schmsol=='SVS' ) then
                    ...
                    p = 1
                    call assign_line_args(svs_mesh%vs%read_oc, args(2), istat)
                end if
            case (VN_SVS_LUNIQUE_PROFILE_SVS2)
                if (.not. svs_mesh%PROCESS_ACTIVE .or. svs_mesh%vs%schmsol=='SVS' ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    p = 1
                    call assign_line_args(svs_mesh%vs%lunique_profile_svs2, args(2), istat)
                end if

            case (VN_SVS_LBCHEAT_SVS2)
                if (.not. svs_mesh%PROCESS_ACTIVE .or. svs_mesh%vs%schmsol=='SVS' ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    p = 1
                    call assign_line_args(svs_mesh%vs%lbcheat_svs2, args(2), istat)
                end if                
            ...
            case (VN_SVS_HSNOWSCHEME)
                if (.not. svs_mesh%PROCESS_ACTIVE  .or. svs_mesh%vs%schmsol=='SVS') then
                    ...
                    p = 1
                    call assign_line_args(svs_mesh%vs%hsnowscheme, args(2), istat)
                end if
            case (VN_SVS_HSNOWDRIFT_CRO)
                if (.not. svs_mesh%PROCESS_ACTIVE  .or. svs_mesh%vs%schmsol=='SVS') then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    p = 1
                    call assign_line_args(svs_mesh%vs%hsnowdrift_cro, args(2), istat)
                end if
            case (VN_SVS_HSNOWMETAMO)
                if (.not. svs_mesh%PROCESS_ACTIVE  .or. svs_mesh%vs%schmsol=='SVS') then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    p = 1
            ...
            case (VN_SVS_HSNOWRAD)
                if (.not. svs_mesh%PROCESS_ACTIVE  .or. svs_mesh%vs%schmsol=='SVS') then
                    ...
                    p = 1
                    call assign_line_args(svs_mesh%vs%hsnowrad, args(2), istat)
                end if
            case (VN_SVS_HSNOWFALL)
                if (.not. svs_mesh%PROCESS_ACTIVE  .or. svs_mesh%vs%schmsol=='SVS') then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    p = 1
                    call assign_line_args(svs_mesh%vs%hsnowfall, args(2), istat)
                end if
            case (VN_SVS_HSNOWCOND)
                if (.not. svs_mesh%PROCESS_ACTIVE  .or. svs_mesh%vs%schmsol=='SVS') then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    p = 1
                    call assign_line_args(svs_mesh%vs%hsnowcond, args(2), istat)
                end if
            case (VN_SVS_HSNOWCOMP)
                if (.not. svs_mesh%PROCESS_ACTIVE  .or. svs_mesh%vs%schmsol=='SVS') then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    p = 1
                    call assign_line_args(svs_mesh%vs%hsnowcomp, args(2), istat)
                end if
            case (VN_SVS_HSNOWHOLD)
                if (.not. svs_mesh%PROCESS_ACTIVE  .or. svs_mesh%vs%schmsol=='SVS') then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    p = 1
                    call assign_line_args(svs_mesh%vs%hsnowhold, args(2), istat)
                end if
            case (VN_SVS_LSNOWDRIFT_SUBLIM)
                if (.not. svs_mesh%PROCESS_ACTIVE .or. svs_mesh%vs%schmsol=='SVS' ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    p = 1
            ...
            case (VN_SVS_LOUT_SNOW_PROFILE)
                if (.not. svs_mesh%PROCESS_ACTIVE .or. svs_mesh%vs%schmsol=='SVS' ) then
                    ...
                    p = 1
                    call assign_line_args(svs_mesh%vs%lout_snow_profile, args(2), istat)
                end if
            ...
            case (VN_SVS_LWRITE_RESTART)
                if (.not. svs_mesh%PROCESS_ACTIVE .or. svs_mesh%vs%schmsol=='SVS' ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    p = 1
                    call assign_line_args(svs_mesh%vs%lwrite_restart, args(2), istat)
                end if
                
            case (VN_SVS_LREAD_RESTART)
                if (.not. svs_mesh%PROCESS_ACTIVE .or. svs_mesh%vs%schmsol=='SVS' ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    p = 1
                    call assign_line_args(svs_mesh%vs%lread_restart, args(2), istat)
                end if
            ...
            case (VN_SVS_CANO_REF_FORCING)
                if (.not. svs_mesh%PROCESS_ACTIVE .or. svs_mesh%vs%schmsol=='SVS' ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    p = 1
                    call assign_line_args(svs_mesh%vs%cano_ref_forcing, args(2), istat)
                end if   
           ...
           case (VN_SVS_HVEGLPOL)
            ...
            case (VN_SVS_SILT)
                if (.not. svs_mesh%PROCESS_ACTIVE) then
                    ...
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_SILT)
                            field2 = adjustl(VN_SVS_SILT_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = shd%lc%IGND
                    call assign_line_args(svs_mesh%vs%silt, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = ignd
                end if
            case (VN_SVS_SILT_N)
                if (.not. svs_mesh%PROCESS_ACTIVE) then
                    ...
                    nargs = nargs - 1
                    b = shd%lc%IGND
                    call assign_line_args(svs_mesh%vs%silt, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = ignd
                end if
            case (VN_SVS_GRAVEL)
                if (.not. svs_mesh%PROCESS_ACTIVE) then
                    ...
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_GRAVEL)
                            field2 = adjustl(VN_SVS_GRAVEL_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = shd%lc%IGND
                    call assign_line_args(svs_mesh%vs%gravel, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = ignd
                end if
            case (VN_SVS_GRAVEL_N)
                if (.not. svs_mesh%PROCESS_ACTIVE) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = shd%lc%IGND
                    call assign_line_args(svs_mesh%vs%gravel, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = ignd
                end if
            case (VN_SVS_BULKSOIL)
                if (.not. svs_mesh%PROCESS_ACTIVE) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_BULKSOIL)
                            field2 = adjustl(VN_SVS_BULKSOIL_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = shd%lc%IGND
                    call assign_line_args(svs_mesh%vs%bulksoil, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = ignd
                end if
            case (VN_SVS_BULKSOIL_N)
                if (.not. svs_mesh%PROCESS_ACTIVE) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = shd%lc%IGND
                    call assign_line_args(svs_mesh%vs%bulksoil, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = ignd
                end if
            case (VN_SVS_OC)
                if (.not. svs_mesh%PROCESS_ACTIVE) then
                    ...
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_OC)
                            field2 = adjustl(VN_SVS_OC_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = shd%lc%IGND
                    call assign_line_args(svs_mesh%vs%oc, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = ignd
                end if
            case (VN_SVS_OC_N)
                if (.not. svs_mesh%PROCESS_ACTIVE) then
                    ...
                    nargs = nargs - 1
                    b = shd%lc%IGND
                    call assign_line_args(svs_mesh%vs%oc, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = ignd
                end if
            case (VN_SVS_WSOIL)
                if (.not. svs_mesh%PROCESS_ACTIVE) then
            ...
            case (VN_SVS_SNOSPHERI_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    ...
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_SNOSPHERI_SVS)
                            field2 = adjustl(VN_SVS_SNOSPHERI_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snospheri_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_SNOSPHERI_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snospheri_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if

            case (VN_SVS_SNOAGEV_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_SNOAGEV_SVS)
                            field2 = adjustl(VN_SVS_SNOAGEV_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snoagev_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_SNOAGEV_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snoagev_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if

            case (VN_SVS_SNOAGE_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_SNOAGE_SVS)
                            field2 = adjustl(VN_SVS_SNOAGE_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snoage_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_SNOAGE_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snoage_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if


            case (VN_SVS_SNODIAMOPTV_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_SNODIAMOPTV_SVS)
                            field2 = adjustl(VN_SVS_SNODIAMOPTV_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snodiamoptv_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_SNODIAMOPTV_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snodiamoptv_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if

            case (VN_SVS_SNODIAMOPT_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_SNODIAMOPT_SVS)
                            field2 = adjustl(VN_SVS_SNODIAMOPT_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snodiamopt_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_SNODIAMOPT_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snodiamopt_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if


            case (VN_SVS_SNOHISTV_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_SNOHISTV_SVS)
                            field2 = adjustl(VN_SVS_SNOHISTV_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snohistv_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_SNOHISTV_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snohistv_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if

            case (VN_SVS_SNOHIST_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_SNOHIST_SVS)
                            field2 = adjustl(VN_SVS_SNOHIST_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snohist_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_SNOHIST_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snohist_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if


                            case (VN_SVS_WSNOWV_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_WSNOWV_SVS)
                            field2 = adjustl(VN_SVS_WSNOWV_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%wsnowv_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_WSNOWV_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%wsnowv_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if

            case (VN_SVS_WSNOW_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_WSNOW_SVS)
                            field2 = adjustl(VN_SVS_WSNOW_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%wsnow_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_WSNOW_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%wsnow_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if


                            case (VN_SVS_TSNOWV_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_TSNOWV_SVS)
                            field2 = adjustl(VN_SVS_TSNOWV_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%tsnowv_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_TSNOWV_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%tsnowv_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if

            case (VN_SVS_TSNOW_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_TSNOW_SVS)
                            field2 = adjustl(VN_SVS_TSNOW_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%tsnow_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_TSNOW_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%tsnow_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if


            case (VN_SVS_SNOMAV_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_SNOMAV_SVS)
                            field2 = adjustl(VN_SVS_SNOMAV_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snomav_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_SNOMAV_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snomav_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if

            case (VN_SVS_SNOMA_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_SNOMA_SVS)
                            field2 = adjustl(VN_SVS_SNOMA_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snoma_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_SNOMA_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snoma_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if
             
            case (VN_SVS_SNODENV_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_SNODENV_SVS)
                            field2 = adjustl(VN_SVS_SNODENV_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snodenv_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_SNODENV_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snodenv_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if

            case (VN_SVS_SNODEN_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_SNODEN_SVS)
                            field2 = adjustl(VN_SVS_SNODEN_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snoden_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_SNODEN_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snoden_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if
            ...
            case (VN_SVS_TPSOIL)
                if (.not. svs_mesh%PROCESS_ACTIVE .or. ( svs_mesh%vs%schmsol == 'SVS' .and. .not. svs_mesh%vs%lsoil_freezing_svs1) ) then
                    ...
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_TPSOIL)
                            field2 = adjustl(VN_SVS_TPSOIL_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
            ...
            case (VN_SVS_TPSOILV)
                if (.not. svs_mesh%PROCESS_ACTIVE .or. svs_mesh%vs%schmsol == 'SVS') then
                    ...
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_TPSOILV)
                            field2 = adjustl(VN_SVS_TPSOILV_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = shd%lc%IGND
                    call assign_line_args(svs_mesh%vs%tpsoilv, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = ignd
                end if
            case (VN_SVS_TPSOILV_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or. svs_mesh%vs%schmsol == 'SVS' ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = shd%lc%IGND
                    call assign_line_args(svs_mesh%vs%tpsoilv, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = ignd
                end if
            case (VN_SVS_TGROUND)
                if (.not. svs_mesh%PROCESS_ACTIVE) then
